...
export type Order = {
  ...
  mealChoice: 'Rice + 4 Rotis' | '5 Rotis';
  date: string;
...
let todaysMenu: MenuItem[] = [allSabjis[1], allSabjis[4]];
...
export const getTodaysMenu = async (): Promise<MenuItem[]> => {
...
export const getAllSabjis = async (): Promise<MenuItem[]> => {
    return new Promise(resolve => setTimeout(() => resolve(allSabjis), 200));
}

export const setTodaysMenu = async (menuItems: MenuItem[]): Promise<void> => {
